...
function injectGoogleFont(fontFamily: string) {
  const fontName = fontFamily.split(',').map(s => s.trim()).join('+');
  if (!document.getElementById(`google-font-${fontName}`)) {
...
function TypographyInjector() {
  const { editorState, isLoading } = useSiteConfig();

  useEffect(() => {
    if (!isLoading) {
      // Apply CSS variables
      document.documentElement.style.setProperty('--font-heading', editorState.typography.headingFont);
      document.documentElement.style.setProperty('--font-body', editorState.typography.bodyFont);

      // Inject Google Fonts
      injectGoogleFont(editorState.typography.headingFont);
      injectGoogleFont(editorState.typography.bodyFont);
    }
  }, [editorState.typography, isLoading]);
