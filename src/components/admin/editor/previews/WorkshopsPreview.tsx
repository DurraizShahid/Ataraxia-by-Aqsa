...
export function WorkshopsPreview() {
  ...
  const workshopSections = Object.values(state.sections)
    .filter((section): section is typeof section & { page: 'workshops' } => section.page === 'workshops')
        ...
        >
          {!section.visible ? (
            ...
            <div className="max-w-[1280px] mx-auto px-6 md:px-16 lg:px-24 py-20">
              <EditableTextOverlay id="workshops-heading">
                <h1
                  style={{
                    ...getTextStyle('workshops-heading'),
                    fontFamily: 'Palatino Linotype, serif',
                  }}
                  className="text-5xl"
                >
                  {state.texts['workshops-heading']?.value || 'Workshops'}
                </h1>
              </EditableTextOverlay>
              <EditableTextOverlay id="workshops-intro">
                <p
                  style={getTextStyle('workshops-intro')}
                  className="mt-6 text-[#A09880] leading-8"
                >
                  {state.texts['workshops-intro']?.value || 'Join our transformative workshops.'}
                </p>
              </EditableTextOverlay>
            </div>
          )}
