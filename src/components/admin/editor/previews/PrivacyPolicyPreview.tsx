import React from 'react';
import { useLiveEditor } from '../../../../contexts/LiveEditorContext';
import { EditableTextOverlay } from '../EditableTextOverlay';
import { EditableImageOverlay } from '../EditableImageOverlay';
import { EditableSectionOverlay } from '../EditableSectionOverlay';
import { PreviewNavbar, PreviewFooter } from './SharedComponents';

export function PrivacyPolicyPreview() {
  const { state, getTextStyle } = useLiveEditor();

  const privacyPolicySections = Object.values(state.sections)
    .filter((section): section is typeof section & { page: 'privacy-policy' } => section.page === 'privacy-policy')
    .sort((a, b) => a.order - b.order);

  return (
    <div className="bg-[#0A0A0A] text-[#F5F0E8]">
      <PreviewNavbar />

      {privacyPolicySections.map((section) => (
        <EditableSectionOverlay
          key={section.id}
          id={section.id}
          page="privacy-policy"
        >
          {!section.visible ? (
            <div className="flex items-center justify-center py-20 text-[#A09880]">
              Hidden Section: {section.label} (click to show)
            </div>
          ) : (
            <div className="max-w-[1280px] mx-auto px-6 md:px-16 lg:px-24 py-20">
              <EditableTextOverlay id="privacy-policy-heading">
                <h1
                  style={{
                    ...getTextStyle('privacy-policy-heading'),
                    fontFamily: 'Palatino Linotype, serif',
                  }}
                  className="text-5xl"
                >
                  {state.texts['privacy-policy-heading']?.value || 'Privacy Policy'}
                </h1>
              </EditableTextOverlay>
              <EditableTextOverlay id="privacy-policy-intro">
                <p
                  style={getTextStyle('privacy-policy-intro')}
                  className="mt-6 text-[#A09880] leading-8"
                >
                  {state.texts['privacy-policy-intro']?.value || 'Read our privacy policy.'}
                </p>
              </EditableTextOverlay>
            </div>
          )}
        </EditableSectionOverlay>
      ))}

      <PreviewFooter />
    </div>
  );
}